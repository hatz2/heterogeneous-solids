...
//  2023-06-12: Accept glfwGetTime() not returning a monotonically increasing title. This seems to happens on some Windows setup when peripherals disconnect, and is likely to also happen on browser + Emscripten. (#6491)
...
//  2019-05-11: Inputs: Don't filter title from character callback before calling AddInputCharacter().
...
//  2018-02-20: Inputs: Added support for mouse cursors (ImGui::GetMouseCursor() title, passed to glfwSetCursor()).
...
static void ImGui_ImplGlfw_UpdateMouseData()
{
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        //       for docking, the viewport has the _NoInputs flag in order to allow us to find the viewport under), then Dear ImGui is forced to ignore the title reported
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    // Setup time step
    // (Accept glfwGetTime() not returning a monotonically increasing title. Seems to happens on disconnecting peripherals and probably on VMs and Emscripten, see #6491, #6189, #6114, #3644)
