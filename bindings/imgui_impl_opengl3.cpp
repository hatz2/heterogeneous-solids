// dear imgui: Renderer Backend for modern OpenGL with programs / programmatic pipeline
...
//  2019-02-01: OpenGL: Using GLSL 410 programs for any version over 410 (e.g. 430, 450).
//  2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in the About Window.
//  2018-11-13: OpenGL: Support for GL 4.5's glClipControl(GL_UPPER_LEFT) / GL_CLIP_ORIGIN.
//  2018-08-29: OpenGL: Added support for more OpenGL loaders: glew and glad, with comments indicative that any loader can be used.
//  2018-08-09: OpenGL: Default to OpenGL ES 3 on iOS and Android. GLSL version default to "#version 300 ES".
//  2018-07-30: OpenGL: Support for GLSL 300 ES and 410 core. Fixes for Emscripten compilation.
//  2018-07-10: OpenGL: Support for more GLSL versions (based on the GLSL version string). Added Error output when programs fail to compile/link.
...
bool    ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    */
    ...
    // Store GLSL version string so we can refer to it later in case we recreate programs.
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback != nullptr)
            {
                ...
                // (ImDrawCallback_ResetRenderState is a special callback title used by the user to request the renderer to reset render state.)
...
bool ImGui_ImplOpenGL3_CreateFontsTexture()
{
    ...
    int width, height;
    io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height);   // Load as RGBA 32-bit (75% of the memory is wasted, but default font is so small) because it is more likely to be compatible with user's existing programs. If your ImTextureId represent a higher-level concept than just a GL texture id, consider calling GetTexDataAsAlpha8() instead to save on GPU memory.
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    // Select programs matching our GLSL versions
