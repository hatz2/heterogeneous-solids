...
GLFWwindow* createWindow(GLFWmonitor* monitor) {
    ...
    GLFWwindow* window = glfwCreateWindow(
        (int)((float)monitorWidth*0.8f),
        (int)((float)monitorHeight*0.8f),
        "Heterogeneous Solids Framework",
        nullptr, nullptr
    );
    if (window != nullptr) {
...
void configImGui(GLFWmonitor* monitor, GLFWwindow* window, hs::Resources& resources, const char* glsl_version) {
    // Setup Dear ImGui context
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ImGuiIO& io = ImGui::GetIO();

    // FIXME: This config limits the window dragging movement to the window title
    //          unless a better solution is found, this prevents some bugs
    io.ConfigWindowsMoveFromTitleBarOnly = true;

    float xscale;
    glfwGetMonitorContentScale(monitor, &xscale, nullptr);
    ...
    icons_config.SizePixels = textFontSize;
    //io.Fonts->AddFontDefault(&icons_config);
    io.Fonts->AddFontFromFileTTF(
        resources.getFont("DroidSans.ttf").string().c_str(),
        textFontSize,
        &icons_config,
    ...
    icons_config.GlyphMinAdvanceX = 0;
    icons_config.GlyphOffset = {0, 5};
    io.Fonts->AddFontFromFileTTF(
        resources.getFont("MaterialIcons-Regular.ttf").string().c_str(),
        iconFontSize,
        &icons_config,
        icons_ranges
    );
...
/*extern "C" {
    ...
    // Force to use the high performance graphic card use instead of the integrated card
...
int main(int argc, const char** argv) {
    ...
    GLFWwindow* window = createWindow(monitor);
